export const determineWinner = (index, board, player) => {
  ...
  let right = savePosiblePositions([x, y + 1, x, y + 2, x, y + 3, x, y + 4]);
  let left = savePosiblePositions([x, y - 1, x, y - 2, x, y - 3, x, y - 4]);
  let down = savePosiblePositions([x + 1, y, x + 2, y, x + 3, y, x + 4, y]);
  let up = savePosiblePositions([x - 1, y, x - 2, y, x - 3, y, x - 4, y]);
  let rightDown = savePosiblePositions([x + 1, y + 1, x + 2, y + 2, x + 3, y + 3, x + 4, y + 4]);
  let rightUp = savePosiblePositions([x - 1, y + 1, x - 2, y + 2, x - 3, y + 3, x - 4, y + 4]);
  let leftUp = savePosiblePositions([x - 1, y - 1, x - 2, y - 2, x - 3, y - 3, x - 4, y - 4]);
  let leftDown = savePosiblePositions([x + 1, y - 1, x + 2, y - 2, x + 3, y - 3, x + 4, y - 4]);
